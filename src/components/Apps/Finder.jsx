...
function Finder({ windowId }) {
  const { 
    ...
    createFile,
    getFile,
    files: fileSystemFiles // Rename to avoid conflict
  } = useFileSystemStore();
  const { 
    desktopFolders, 
    desktopFiles, 
    ...
    addDesktopFile, 
    removeDesktopItem,
    updateDesktopItemName,
    updateDesktopItemPosition // Assuming this exists or will be added
  } = useDesktopStore();
  ...
  // Local state for downloads, will be synced with fileSystemStore if needed
  const [downloads, setDownloads] = useState(() => {
    const saved = localStorage.getItem("os_downloads");
    return saved ? JSON.parse(saved) : [];
  });

  // Sidebar items - adapted to use existing store structure
  const sidebarItems = [
    { id: "favorites", label: "Favorites", type: "header" },
    { id: "downloads", label: "Downloads", icon: FiDownload, path: "/Downloads" }, // Path to match fileSystemStore
    { id: "documents", label: "Documents", icon: BsFolder, path: "/Documents" },
    { id: "pictures", label: "Pictures", icon: BsImage, path: "/Pictures" },
    { id: "macintosh-hd", label: "Macintosh HD", icon: BsFolder, path: "/" },
  ];

  // Listen for new downloads (from Safari/Photos)
  useEffect(() => {
    const handleDownload = (e) => {
      ...
      setDownloads(prev => {
        ...
        // Optionally, also add to fileSystemStore under Downloads
        createFile(newFile.name, newFile.url, newFile.type, '/Downloads'); // Assuming createFile can take a path
        return updated;
      });
    };
  ...
  const getCurrentFolderName = () => {
    ...
    if (lastPart) {
      const folder = getCurrentFolder(); // Get folder from fileSystemStore
      return folder ? folder.name : lastPart;
    }
    return "Macintosh HD";
  };
  ...
  const getFilesForCurrentPath = () => {
    ...
    const childrenArray = Object.values(currentFolderData.children);

    // Special handling for "Downloads" if it's a flat list
    if (currentPath === "/Downloads") {
      return downloads.map(d => ({
        ...
        date: d.date,
        size: d.size,
        extension: d.name.split('.').pop() // Add extension for generic file icon
      }));
    }

    return childrenArray.map(item => ({
      id: item.name, // Use name as ID for simplicity in this context
      name: item.name,
      type: item.type,
      content: item.content,
      extension: item.extension,
      url: item.url, // For images
  ...
  const deleteFile = (fileId) => {
    // Find the file to delete from downloads
    ...
    // Also delete from fileSystemStore if it was there
    deleteItem(fileToDelete.name); // Assuming deleteItem works with name
  };
  ...
  const handleCopyToDesktopFolder = (file, folder) => {
    // This logic needs to be adapted to use useFileSystemStore and useDesktopStore
    // For now, it's a placeholder
    console.log(`Copying ${file.name} to desktop folder ${folder.name}`);
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {selectedFile && (
          <div className="flex items-center justify-between px-4 py-2 border-t bg-gray-50/80" style={{ borderColor: "rgba(224, 224, 224, 0.5)" }}>
            ...
            <div className="flex items-center gap-2">
              {/* Removed desktopFolders related logic for now, needs proper integration */}
              <button
