...
export default function LockScreen({ goNext }) {
  ...
  const [lockscreenWallpaper, setLockscreenWallpaper] = useState("/wallpaper/sana'sKitty.jpg");
  // User profile state
  const [username, setUsername] = useState(() => localStorage.getItem("lock_username") || "Kumar Vikalp");
  const [profilePhoto, setProfilePhoto] = useState(() => localStorage.getItem("lock_profile_photo") || "/wallpaper/Vikalp.jpg"); // Pexels URL
  ...
  useEffect(() => {
    const img = new Image();
    img.src = localStorage.getItem("lockscreen_wallpaper") || "https://images.pexels.com/photos/167699/pexels-photo-167699.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"; // Pexels URL
  }, []);
